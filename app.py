...
(function(){
  ...
  window.acT=function(m){
    localStorage.setItem('ac_theme',m);
    setT(m);
    // Disparar también el tema nativo de Streamlit
    var stVal=m==='auto'?null:JSON.stringify({name:m==='dark'?'Dark':'Light'});
    if(stVal) localStorage.setItem('stActiveTheme',stVal);
    else localStorage.removeItem('stActiveTheme');
    window.dispatchEvent(new StorageEvent('storage',{key:'stActiveTheme',newValue:stVal,storageArea:localStorage,bubbles:true}));
  };
