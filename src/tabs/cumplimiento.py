...
def _mostrar_chat_cumplimiento(chatbot: AIComplyChat) -> None:
    ...
    mensajes_c = st.session_state.mensajes_cumplimiento
    if len(mensajes_c) >= 2 and mensajes_c[-1]["role"] == "assistant":
        if st.button("↻ Regenerar respuesta", key="btn_regen_cump"):
            st.session_state.mensajes_cumplimiento = mensajes_c[:-2]
...
def mostrar_tab_cumplimiento(provider: LLMProvider) -> None:
    ...
    # ── EXCLUIDO / NO CUMPLE LA DEFINICIÓN: no procede cumplimiento ────────────
    clasificacion_actual = (st.session_state.get("clasificacion_data") or {}).get("clasificacion", "").upper()
    if evaluacion_completada and es_sin_obligaciones(clasificacion_actual):
        st.info(
            f"{texto_sin_obligaciones(clasificacion_actual)} "
